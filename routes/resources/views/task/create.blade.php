...
@section('page-description', 'Create a new task with assignment and priority details')
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0 rounded-lg">
                <div class="card-header bg-white border-bottom py-3">
                    <h4 class="card-title mb-0 text-dark font-weight-bold">Create Task</h4>
                </div>
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('tasks.store') }}" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('assigned_team') is-invalid @enderror" id="assigned_team" name="assigned_team" required>
                                    ...
                                    <option value="Individual" {{ old('assigned_team') == 'Individual' ? 'selected' : '' }}>Individual</option>
                                    <option value="Team" {{ old('assigned_team') == 'Team' ? 'selected' : '' }}>Team</option>
                                </select>
                                @error('assigned_team')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="individual-fields" style="display: none;">
                            <div class="mb-3">
                                <label for="assigned_to" class="form-label fw-bold">Assigned To <span class="text-danger">*</span></label>
                        ...
                        <div class="team-fields" style="display: none;">
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select @error('team_members') is-invalid @enderror" id="team_members" name="team_members[]" multiple>
                                        @foreach($employees as $employee)
                                            <option value="{{ $employee->id }}" {{ in_array($employee->id, old('team_members', [])) ? 'selected' : '' }}>
                                                {{ $employee->name }} ({{ $employee->employee_code }})
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="mb-3">
                                <label for="team_created_by" class="form-label fw-bold">Team Created By <span class="text-danger">*</span></label>
                                <select class="form-select @error('team_created_by') is-invalid @enderror" id="team_created_by" name="team_created_by">
                                    <option value="">Select Option</option>
                                    <option value="admin" {{ old('team_created_by') == 'admin' ? 'selected' : '' }}>Admin</option>
                                    <option value="team_lead" {{ old('team_created_by') == 'team_lead' ? 'selected' : '' }}>Team Lead</option>
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('progress') is-invalid @enderror" id="progress" name="progress" value="{{ old('progress', 0) }}" min="0" max="100">
                            @error('progress')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
