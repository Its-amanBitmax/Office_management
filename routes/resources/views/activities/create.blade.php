...
@section('page-description', 'Add a new activity to the system')
                ...
                <div class="card-body">
                    <form action="{{ route('activities.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select multiple class="form-select @error('employees') is-invalid @enderror" id="employees" name="employees[]">
                                @foreach($employees as $employee)
                                    <option value="{{ $employee->id }}" {{ (collect(old('employees'))->contains($employee->id)) ? 'selected' : '' }}>
                        ...
                        <div class="mb-3 form-check">
                            ...
                            <input type="checkbox" class="form-check-input @error('enable_best_employee') is-invalid @enderror" id="enable_best_employee" name="enable_best_employee" value="1" {{ old('enable_best_employee') ? 'checked' : '' }}>
...
-                        <div id="best-employee-section" style="display: none;">
-                            <h5 class="mb-3">Best Employee Selection</h5>
-
-                            {{-- Best Employee --}}
-                            <div class="mb-3">
-                                <label for="best_employee_id" class="form-label">Best Employee</label>
-                                <select class="form-select @error('best_employee_id') is-invalid @enderror" id="best_employee_id" name="best_employee_id">
-                                    <option value="">Select Best Employee</option>
-                                    @foreach($employees as $employee)
-                                        <option value="{{ $employee->id }}" {{ old('best_employee_id') == $employee->id ? 'selected' : '' }}>
-                                            {{ $employee->name }} ({{ $employee->employee_code }})
-                                        </option>
-                                    @endforeach
-                                </select>
...
-                            </div>
-
-                            {{-- Best Employee Description --}}
-                            <div class="mb-3">
-                                <label for="best_employee_description" class="form-label">Best Employee Description</label>
-                                <textarea class="form-control @error('best_employee_description') is-invalid @enderror" id="best_employee_description" name="best_employee_description" rows="3">{{ old('best_employee_description') }}</textarea>
...
-                            {{-- Keep Best Employee --}}
-                            <div class="mb-3 form-check">
-                                <input type="checkbox" class="form-check-input @error('keep_best_employee') is-invalid @enderror" id="keep_best_employee" name="keep_best_employee" value="1" {{ old('keep_best_employee') ? 'checked' : '' }}>
-                                <label class="form-check-label" for="keep_best_employee">Keep Best Employee</label>
-                                @error('keep_best_employee')
-                                    <div class="invalid-feedback">{{ $message }}</div>
-                                @enderror
-                            </div>
-                        </div>
+                        {{-- Removed detailed best employee fields as per user request --}}

                        {{-- Schedule --}}
                        <div class="mb-3">
                            <label for="schedule_at" class="form-label">Schedule Date & Time</label>
                            <input type="datetime-local" class="form-control @error('schedule_at') is-invalid @enderror" id="schedule_at" name="schedule_at" value="{{ old('schedule_at') }}">
                            @error('schedule_at')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div id="criteria-wrapper">
                                <div class="criteria-item border p-3 mb-2 rounded">
                                    <div class="mb-2">
                                        <input type="text" name="criteria[0][name]" class="form-control" placeholder="Criteria Name (e.g. Communication)" required>
                                    </div>
                                    <div class="mb-2">
                                        <textarea name="criteria[0][description]" class="form-control" rows="2" placeholder="Criteria Description"></textarea>
                                    </div>
                                    <div>
                                        <input type="number" name="criteria[0][max_points]" class="form-control" placeholder="Max Points (e.g. 10)" required>
                                    </div>
                                </div>
                            </div>
